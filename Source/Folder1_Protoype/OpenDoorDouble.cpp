...
void UOpenDoorDouble::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (PressurePlate->IsOverlappingActor(ActorThatOpens) && PressurePlate->IsOverlappingActor(SecondActorThatOpens))
	{
		FVector ActorLocation = GetOwner()->GetActorLocation();
		Counter = true;
		...
		float DoorIsClosed = ActorLocation.Z - 400.0f;
	...
	else
	{
		...
		float DoorIsOpen = ActorLocation.Z + 400.0f;
